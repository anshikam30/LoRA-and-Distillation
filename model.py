...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.lora_c_fc = LoRALinear(config.n_embd, 4 * config.n_embd, config.LoRA_rank, alpha=1)
        self.lora_c_proj = LoRALinear(4*config.n_embd, config.n_embd, config.LoRA_rank, alpha=1)
...
class LoRALinear(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, rank, alpha) -> None:
        super().__init__()
        self.B = torch.nn.Parameter(torch.zeros(input_dim, rank))
        self.A = torch.nn.Parameter(torch.randn(rank, output_dim))
        self.alpha = alpha
        self.scaling = alpha/rank
    def forward(self, x):
        return x@(self.B @ self.A)*self.scaling
...
class DistilRNN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_layers=1, vocab_size=50304 ):
        super().__init__()
    ...
    def forward(self, x):
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_dim, dtype=torch.float32).to(x.device)
        x = self.emb(x)
